...
async def create_shop_help() -> str:
    ...
    for idx, goods in enumerate(_list):
        ...
        await name_image.aline((380, -5, 280, 45), "#FFDEAD", 10)   #棕色
        ...
        des_image = None
        font_img = BuildImage(
            600, 80, font_size=20, color="#FF3030", font="CJGaoDeGuo.otf" #红色
        )
        ...
        if goods.goods_description:
            ...
            if desc[-1] == "\n":
                desc = desc[:-1]
            des_image = await text2image(desc, color="#CD5C5C") #介绍深棕色
        ...
        await goods_image.acircle_corner(30)
        bk = BuildImage(
            1180,
            (50 + des_image.h) if des_image else 50,
            font_size=15,
        ...
        if total_n < n:
            total_n = n
        if n:
            await bk.aline((650, -1, 650 + n, -1), "#191970", 5) #深蓝色
    ...
    A = BuildImage(1000, h, color="#FFFAFA") #  长宽
    ...
    shop = BuildImage(w, h, font_size=20, color="#FFFAFA") #白色
    zx_img = BuildImage(0, 0, background=f"{IMAGE_PATH}/tuku_10/商店壁纸.png") #背景
    zx_img.transpose(Image.FLIP_LEFT_RIGHT)
    zx_img.replace_color_tran(((240, 240, 240), (255, 255, 255)), (249, 246, 242))
    await shop.apaste(zx_img, (0, 0))  #壁纸位置
    ...
    shop.text((20 + shop_logo.w, h - 100), "神秘药水\t\t售价：10000000金币\n\t\t据说可以对喜欢的人进行涩涩~")
    return shop.pic2bs4()
